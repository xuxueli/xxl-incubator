...
序号| 项目       | 协议              | 特征            |  特征
----|----------|-----------------|---------------|-------------- 
1   | xxl-job  | GPL             | 任务调度平台        |    轻量级、分布式、多语言、多模式（GLUE）
2   | xxl-mq   | GPL             | 延时消息队列        |    延时、重试、分片、可追溯 
3   | xxl-conf | GPL             | 配置及注册中心       |   配置中心：秒级推送、一致性、安全(灰度、可观测)；注册中心：动态注册、实时推动； 
4   | xxl-sso  | GPL             | 单点登录框架        |    基础登录、鉴权(sdk化)、单点登录；（IAM + SSO）               【IAM身份认证：租户空间、权限定义、用户管理/授权； SSO认证中心，集成；】
5   | xxl-api  | GPL             | API管理平台       |     API管理、权限隔离、Mock Server、自动化测试、swagger导入导出；
6   | xxl-boot | GPL             | 中后台脚手架        |    快速开发框架、单体式、前后端分离；                            【插件包：AI插件；工作流插件；】
